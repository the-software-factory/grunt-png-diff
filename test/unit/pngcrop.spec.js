describe('PNG crop\'s', function () {
  ...
  var PNGCrop = require('../../lib/pngcrop');

  var img1Path = __dirname + '/fixtures/1.png';
  var imgExpectedCropOverflowPath = __dirname + '/fixtures/expectedCropOverflow.png';
  var imgExpectedCropPath = __dirname + '/fixtures/expectedCrop.png';
  var imgExpectedCropTopLeftConfigPath =
    __dirname + '/fixtures/expectedCropTopLeftConfig.png';
  ...
  describe('crop', function () {
    ...
    it('should error for misinput', function (done) {
      PNGCrop.crop('bla', 'dest', {}, function (err) {
        expect(err.message).toEqual('Image cropping dimensions should specify width and height');
        done();
      });
    });
  ...
  describe('cropToStream', function () {
    ...
    it('should require width and height info', function (done) {
      PNGCrop.cropToStream(img1Path, {}, function (err) {
        expect(err.message).toEqual('Image cropping dimensions should specify width and height');
        done();
      });
    });
